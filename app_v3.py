# ── Smart Urban Energy Efficiency System v3 ──────────────────────────────────
# New version with Login Page + Summary Page
# ─────────────────────────────────────────────────────────────────────────────
...
def show_login():
    ...
    st.markdown('<p class="sub-title">Bangalore Urban Zone Monitoring Portal</p>',
                unsafe_allow_html=True)
    st.markdown("---")
    ...
    with col2:
        ...
        location = st.selectbox("📍 Your Location", [
            ...
            "Hebbal", "Jayanagar", "Rajajinagar",
            "Electronic City", "Yeshwanthpur", "Other"])
        ...
        if login_btn:
            ...
            elif location == "Select your location":
...
def show_summary():
    ...
    st.markdown(
        ...
        f'📍 {st.session_state.user_loc} &nbsp;|&nbsp; '
        f'🕐 Logged in at {st.session_state.login_time}'
        f'</div>', unsafe_allow_html=True)

    st.markdown("## 📋 System Summary")
    st.markdown("A quick snapshot of the Smart Urban EE System status.")
    st.markdown("---")

    # ── Section 1: System Health ──────────────────────────────────────────────
    st.markdown("### 1️⃣ System Health at a Glance")
    c1, c2, c3, c4 = st.columns(4)
    total_readings  = len(df_elec) + len(df_water)
    total_anomalies = (df_elec['combined_anomaly'].sum() +
                       df_water['combined_anomaly'].sum())
    critical_count  = ((df_elec['severity']=='Critical').sum() +
                       (df_water['severity']=='Critical').sum())
    normal_pct      = round((1 - (total_anomalies/total_readings))*100, 1)

    c1.metric("📊 Total Readings",    f"{total_readings:,}")
    c2.metric("⚠️ Total Anomalies",   f"{total_anomalies:,}")
    c3.metric("🔴 Critical Alerts",   f"{critical_count:,}")
    c4.metric("✅ System Normal %",   f"{normal_pct}%")

    st.markdown("---")

    # ── Section 2: Most Urgent Alert ─────────────────────────────────────────
    st.markdown("### 2️⃣ Most Urgent Alert Right Now")

    top_elec  = elec_report[elec_report['critical_count']>0].head(1)
    top_water = water_report[water_report['critical_count']>0].head(1)

    col1, col2 = st.columns(2)
    with col1:
        if not top_elec.empty:
            r = top_elec.iloc[0]
            st.markdown(
                f'<div class="urgent-alert">'
                f'<h4>⚡ Electricity — Critical Location</h4>'
                f'<b>📍 {r["zone"]} → {r["ward"]} → {r["block"]}</b><br>'
                f'🔴 Critical Anomalies : {int(r["critical_count"])}<br>'
                f'⚠️ Total Anomalies : {int(r["total_anomalies"])}<br>'
                f'🔍 Top Cause : {r["top_cause"]}<br>'
                f'<b>Action : Send technician immediately!</b>'
                f'</div>', unsafe_allow_html=True)
        else:
            st.success("✅ No critical electricity alerts!")

    with col2:
        if not top_water.empty:
            r = top_water.iloc[0]
            st.markdown(
                f'<div class="urgent-alert">'
                f'<h4>💧 Water — Critical Location</h4>'
                f'<b>📍 {r["zone"]} → {r["ward"]} → {r["block"]}</b><br>'
                f'🔴 Critical Anomalies : {int(r["critical_count"])}<br>'
                f'⚠️ Total Anomalies : {int(r["total_anomalies"])}<br>'
                f'🔍 Top Cause : {r["top_cause"]}<br>'
            ...
            st.success("✅ No critical water alerts!")

    st.markdown("---")

    # ── Section 3: Quick Model Verdict ───────────────────────────────────────
    st.markdown("### 3️⃣ Model Performance Verdict")

    ze_f1 = f1_score(df_elec['is_anomaly'],
                     df_elec['zscore_anomaly']) * 100
    if_f1 = f1_score(df_elec['is_anomaly'],
                     df_elec['if_anomaly'])     * 100
    better = ("Isolation Forest" if if_f1 > ze_f1 else "Z-Score")

    col3, col4, col5 = st.columns(3)
    col3.metric("📐 Z-Score F1 Score",          f"{ze_f1:.2f}%")
    col4.metric("🤖 Isolation Forest F1 Score",  f"{if_f1:.2f}%")
    col5.metric("🏆 Better Model",               better)

    st.markdown("---")

    # ── Section 4: Resource Split ─────────────────────────────────────────────
    st.markdown("### 4️⃣ Anomaly Split — Electricity vs Water")
    col6, col7 = st.columns(2)

    with col6:
        elec_anom  = int(df_elec['combined_anomaly'].sum())
        water_anom = int(df_water['combined_anomaly'].sum())
        fig = go.Figure(go.Pie(
            labels=['⚡ Electricity','💧 Water'],
            values=[elec_anom, water_anom],
            hole=0.4,
            marker_colors=['#e74c3c','#3498db']))
        fig.update_layout(height=300,
                          title="Anomaly Distribution by Resource")
        st.plotly_chart(fig, use_container_width=True)

    with col7:
        st.markdown("#### Quick Stats")
        st.markdown(f"- ⚡ Electricity Anomalies : **{elec_anom:,}**")
        st.markdown(f"- 💧 Water Anomalies : **{water_anom:,}**")
        st.markdown(f"- 🏙️ Zones Monitored : **8**")
        st.markdown(f"- 🗺️ Wards Monitored : **24**")
        st.markdown(f"- 🔲 Blocks Monitored : **96**")
        st.markdown(f"- 📅 Data Period : **Jan–Mar 2022**")
        st.markdown(f"- 🤖 ML Models Used : **Z-Score + Isolation Forest**")

# ══════════════════════════════════════════════════════════════════════════════
# MAIN DASHBOARD
# ══════════════════════════════════════════════════════════════════════════════
def show_dashboard():
    # Welcome bar on every page
    st.markdown(
        f'<div class="welcome-bar">'
        f'👤 <b>{st.session_state.user_name}</b> &nbsp;|&nbsp; '
        f'📍 {st.session_state.user_loc} &nbsp;|&nbsp; '
    ...
    if page == "📋 Summary":
        ...
        with col6:
                                  ...
                                  value=pd.to_datetime("2022-01-07"))
        ...
        ref      = df_ref[(df_ref['location_id']==m_loc) &
                          (df_ref['hour']==m_hour)]
